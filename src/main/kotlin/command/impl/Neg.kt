...
class Neg : ListenerAdapter() {
    ...
    override fun onGuildMessageReceived(event: GuildMessageReceivedEvent) {
        run {
            ...
            if (command.size < 3) {
                errorResponse(event, "Correct Syntax: `::neg @user [Brief Comment About User]`")
                return
            }
            ...
            target?.let {
                ...
                if (Reputation.checkIfRepped(event.author.id, target.id)) {
                    Reputation.modifyRep(event.author.id, target.id, false, comment)

                    val embed = Helper.createEmbed("Reputation Modified")
                    embed.setDescription("${event.author.asTag} negged ${target.asTag}")
                ...
                val embed = Helper.embed(event, "User Negged")
                embed.setDescription("${event.author.asTag} negged ${target.asTag} for $amount points!")
                embed.addField("Comment:", comment, false)
